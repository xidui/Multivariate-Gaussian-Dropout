...
class LeNet(nn.Module):
	def __init__(self, conv_feature_1=6, conv_feature_2=16, fc1=500, fc2=200, dropout=None, input_dropout_mask = None):
		...
		self.conv1_dropout_mask = None
		self.conv2_dropout_mask = None
		self.fc1_dropout_mask = None
	...
	def forward(self, x):
		...
		self.set_dropout_masks(x)

		x_dropout = x * self.input_dropout_mask

		out = F.relu(self.conv1(x_dropout))  # (batch_size * 6 * 28 * 28)
...
if __name__ == '__main__':
	...
	for epoch in range(500):
		...
		for id, (inputs, labels) in enumerate(test_loader):
			...
			print('Epoch: {4} | Loss: {0} | Acc: {1} ({2}/{3}) | test'.format(test_loss / (id + 1), 100. * test_correct / test_total, test_correct, test_total, epoch))

		print(f, 'Epoch: {0} | Train Loss: {1} | Test Loss: {2} | Train Acc: {3} | Test Acc: {4}'.format(epoch, train_loss / train_total, test_loss / test_total, 100. * train_correct / train_total, 100. * test_correct / test_total))
